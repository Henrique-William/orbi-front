...
export default function RoutesPage() {
    ...
    useEffect(() => {
        const fetchRoutes = async () => {
            // O token não é lido pelo JS, mas será enviado pelo navegador.
            try {
                setLoading(true);
                const response = await fetch(`${BASE_URL}/api/route`, {
                    ...
                    // ADICIONADO: Força o navegador a incluir cookies HttpOnly na requisição
                    credentials: 'include',
                });

                if (response.status === 401 || response.status === 403) {
                    // O backend devolveu 401/403 (Token não enviado ou inválido/expirado)
                    setError('Acesso negado (401/403). Por favor, faça login novamente.');
                ...
                // Se o erro foi definido como 401/403 acima, mantemos essa mensagem mais específica
                if (!error) {
    ...
    if (loading) {
        return <div className="flex h-screen items-center justify-center bg-[#f8f8f8] text-black">Carregando Rotas...</div>;
    }
